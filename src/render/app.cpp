...
void ToolBar::set_tools(std::vector<Tool>* new_tools) {
    tools_ = new_tools;
}


void ToolBar::render_impl(Renderer* ren) {
    ...
    for (int i=0; i<tools_->size(); ++i) {
        ...
        button.y = style_.y_begin + i*(style_.icon_size + style_.icon_padding);

        if (GuiButton(button, NULL)) {
            tool.on_click();
        }

        if (selected_.value_or(-1) == i) {
            DrawRectangleLinesEx(button, 2.0f, RED);
        }

        GuiDrawIcon(tool.icon, button.x, button.y,
                style_.icon_pixel_size, BLACK);
    }
}


void App::run_generator() {
    gen_.generate();
    for (int i=0;i<gen_.road_type_count();++i)
        std::cout << gen_.road_count(i, Eigenfield::major()) + gen_.road_count(i, Eigenfield::minor()) << std::endl;
}
...
void App::set_state(AppState s) {
    toolbar.set_tools(&tools[s]);
    app_state_ = s;
}
...
void App::main_loop() {
    ...
    ren_.begin_drawing(); {
        ...
        if (app_state_ == Editor) 
            field_view.render(&ren_);

        ren_.begin_mode_2d(); {
            if (app_state_ == Map)
                map_view.render_2d(&ren_);
        } ren_.end_mode_2d();
